...
test('BotRow keeps turn-busy (work mood) a socket fact', () => {
  ...
  assert.match(row, /const botMood = studioFleetWorking \|\| ccdWorking \|\| workerActive \|\| \(isGatewayHome && gatewayState === 'busy'\) \? 'work' : 'idle'/)
})
...
test('Cody and CCD surfaces publish source-qualified selection and workspace ownership', () => {
  for (const functionName of ['openStudioFleetCodyWorkspace', 'openCcdWorkspace']) {
    const start = source.indexOf(`function ${functionName}(`)
    assert.ok(start >= 0)
    const body = source.slice(start, start + 1200)
    assert.match(body, /saveSelectedRosterBot\(bot\)/)
    assert.match(body, /setBotsWorkspaceOwner\(botWorkspaceOwnerKey\(bot\), bot\)/)
  }
  assert.doesNotMatch(source, /\$botWorkspaceProfile/)
})
