...
test('overlay is scoped to the existing canonical cody profile only', () => {
  ...
  assert.match(source, /const codyWorkVisible = isStudioFleetCody && \['working', 'queued'\]\.includes/)
  assert.match(source, /children: 'Today’s work'/)
  assert.match(source, /async function openCodyConversation/)
  assert.match(source, /openBotCanonicalChat\('cody', pinned/)
  assert.match(source, /creating a second chat or sending anything into the hardened harness/)
  assert.match(source, /retained work stays available through the explicit context-menu action/)
