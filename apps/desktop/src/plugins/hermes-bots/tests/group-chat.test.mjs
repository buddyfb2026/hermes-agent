...
test('Bot-row navigation closes the selected group main tab before opening chat', () => {
  ...
  assert.equal(closed, 1)
  assert.equal(fronted, 1)
  assert.match(pluginSource, /async function openRosterBot\(bot\)[\s\S]*?dismissGroupChatForLocalBotOpen\(\)/)
})
...
test('main workspace and legacy pane fallback are mutually exclusive', () => {
  const modern = load(() => '(pass)')
  modern.host.openWorkspace = () => () => undefined
  modern.openGroupChat('BIZ-1066')
  assert.equal(modern.shouldRenderGroupChatInPane('BIZ-1066'), false)

  const legacy = load(() => '(pass)')
  legacy.host.openWorkspace = undefined
  legacy.openGroupChat('BIZ-1066')
  assert.equal(legacy.shouldRenderGroupChatInPane('BIZ-1066'), true)
})
...
test('source contract: group roster rows expose a confirmed Delete Group action', () => {
  assert.match(pluginSource, /function GroupRow\(\{ active, group, members, needsYou, archived, onOpen, onDisband \}\)/)
