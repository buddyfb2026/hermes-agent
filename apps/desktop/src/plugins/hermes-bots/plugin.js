...
const previous = $lastRoster.get().filter(row => !row?.ghost)
      ...
      return withCcdWorkerRow({ ...(local && typeof local === 'object' ? local : {}), fetchedAt: issuedAt })
    },
...
async function openRosterBot(bot) {
  ...
  if (isCcdWorker) {
    ...
    return openCcdWorkspace()
  }
  closeCcdWorkspace()
  const codyWorkVisible = isStudioFleetCody && ['working', 'queued'].includes($studioFleetCody.get().state)
  if (codyWorkVisible && openStudioFleetCodyWorkspace()) {
    return true
  }
  if (isStudioFleetCody) closeStudioFleetCodyWorkspace()
...
function groupChatMemberBots(group, roster, metaByName) {
  ...
  for (const descriptor of stored) {
    ...
    seated.add(key)
    const live = (roster || []).find(bot => !bot?.ghost && botRosterKey(bot) === key)
    const chosen = live || resolved
    remote.push(live && descriptor.externalWorker ? { ...chosen, externalWorker: descriptor.externalWorker } : chosen)
  }
...
function openCcdWorkspace() {
  const bot = $lastRoster.get().find(candidate => candidate.externalWorker === 'ccd')
  if (bot) {
    saveSelectedRosterBot(bot)
    setBotsWorkspaceOwner(botWorkspaceOwnerKey(bot), bot)
  }
...
const studioFleetCody = useValue($studioFleetCody)
  ...
  const isStudioFleetCody = !bot.remoteSource && bot.name === 'cody'
  const isCcdWorker = bot.externalWorker === 'ccd'
  // Preview identity must match click identity (#88200).
  ...
  const botMood = studioFleetWorking || ccdWorking || workerActive || (isGatewayHome && gatewayState === 'busy') ? 'work' : 'idle'
    ...
    isStudioFleetCody && ['working', 'queued', 'unavailable'].includes(studioFleetCody.state)
      ...
      : stripPreviewMarkdown(
          fromBot
            ...
            : previewSession?.preview || bot.description || ''
        )
...
function useRoutines(owner, enabled = true) {
  const bot = typeof owner === 'string' ? { name: owner } : owner
  const route = botConnectionRoute(bot)
  ...
  return useQuery({
    ...
    enabled: enabled && Boolean(bot?.name),
...
function RoutinesPane() {
  ...
  const isCcd = owner?.externalWorker === 'ccd' || bot === 'ccd'
  const { shape, color, image } = botAppearance(bot, meta)
  const { data, error, isLoading, refetch } = useRoutines(owner, !isCcd)
...
function GroupRow({ active, group, members, needsYou, archived, onOpen, onDisband }) {
  const rooms = useValue($groupChats)
...
function BotsPane() {
  ...
  const groupNames = groupChatNames(allMeta, groupRooms, { includeArchived: showArchivedGroups })
  const groupRows = groupNames
    ...
    .map(row => ({
      ...
      archived: groupRooms[row.name]?.lifecycle?.state === 'archived',
      active:
        Boolean(
